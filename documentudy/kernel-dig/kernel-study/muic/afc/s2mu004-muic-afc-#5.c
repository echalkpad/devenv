		...
		(struct s2mu004_muic_data *muic_data, const muic_afc_data_t *new_afc_data)
{
	...
	/* cancel_delayed_work(&muic_data->afc_check_interrupt); */
...
//		s2mu004_hv_muic_adcmode_oneshot(muic_data);
	...
	/* schedule_delayed_work(&muic_data->afc_check_interrupt, msecs_to_jiffies(2000)); */
...
void s2mu004_muic_afc_irq_detect(struct work_struct *work)
{
	struct s2mu004_muic_data *muic_data =
		container_of(work, struct s2mu004_muic_data, afc_irq_detect.work);
//	u8 data[2];
	pr_err("[DEBUG] %s(%d) \n " , __func__, __LINE__);
//	data[0] = 0xe0;
//	data[1] = 0x06;
//	s2mu004_bulk_write(muic_data->i2c, 0x49, 2, data);
	s2mu004_hv_muic_write_reg(muic_data->i2c, 0x49, 0xE0);
	s2mu004_hv_muic_write_reg(muic_data->i2c, S2MU004_REG_AFC_CTRL2, S2MU004_MUIC_HVCONTROL2_06);
}

void s2mu004_muic_afc_send_mpnack(struct work_struct *work)
{
	struct s2mu004_muic_data *muic_data =
		container_of(work, struct s2mu004_muic_data, afc_check_interrupt.work);
	pr_err("[DEBUG] %s(%d) \n " , __func__, __LINE__);
	s2mu004_hv_muic_detect_dev(muic_data, muic_data->irq_mpnack);
}

// static int count_mrx=0;
void s2mu004_muic_afc_mrxrdy(struct work_struct *work)
{
	struct s2mu004_muic_data *muic_data =
		container_of(work, struct s2mu004_muic_data, afc_mrxrdy.work);
	pr_err("[DEBUG] %s(%d) \n " , __func__, __LINE__);
...
//			__func__);
	if (muic_data->is_afc_muic_ready == false)
...
#if 0
		schedule_delayed_work(&muic_data->afc_mrxrdy, 0);
#endif
		...
		{
			pr_err("[DEBUG] %s: VbADC interrupt after sending master ping x3\n " , __func__);
			...
			if (val_vbadc != VBADC_8_7V_9_3V) {
				pr_err("[DEBUG] %s: vbadc: %d\n " , __func__, val_vbadc);
				schedule_delayed_work(&muic_data->afc_check_vbadc, msecs_to_jiffies(30));
			}
		}
...
//	s2mu004_hv_muic_write_reg(muic_data->i2c, 0xC6, 0x08); /* OTP */
//	s2mu004_muic_hv_update_reg(muic_data->i2c, 0x6e, 0x02, 0x03, 1);
	...
	INIT_DELAYED_WORK(&muic_data->afc_irq_detect, s2mu004_muic_afc_irq_detect);
	INIT_DELAYED_WORK(&muic_data->afc_check_interrupt, s2mu004_muic_afc_send_mpnack);
//	INIT_DELAYED_WORK(&muic_data->afc_send_mpnack, s2mu004_muic_afc_send_mpnack);

	INIT_DELAYED_WORK(&muic_data->afc_mrxrdy, s2mu004_muic_afc_mrxrdy);
	mutex_init(&muic_data->afc_mutex);
//	INIT_DELAYED_WORK(&muic_data->hv_muic_qc_vb_work, s2mu004_hv_muic_check_qc_vb);
//	INIT_DELAYED_WORK(&muic_data->hv_muic_mping_miss_wa, s2mu004_hv_muic_check_mping_miss);
}
